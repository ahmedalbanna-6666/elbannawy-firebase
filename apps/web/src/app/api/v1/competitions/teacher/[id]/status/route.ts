...
export async function PATCH(
  ...
  try {
    ...
    if (!['OPEN', 'CLOSED', 'FINALIZED'].includes(status)) {
      return NextResponse.json({ success: false, error: { code: 'INVALID_INPUT', message: 'Invalid status. Must be OPEN, CLOSED, or FINALIZED' } }, { status: 400 });
    }

    const result = await competitionRepo.update(id, { status } as any);
    if (!result.ok) {
