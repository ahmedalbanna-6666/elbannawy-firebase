...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    let body: { targetType?: string; cost?: number };
    try { body = await request.json() as typeof body; } catch {
      return NextResponse.json({ success: false, error: { code: 'INVALID_INPUT', message: 'Invalid JSON body' } }, { status: 400 });
    }
    const targetType = (body.targetType ?? '').toUpperCase();
    const cost = Number(body.cost) || 0;
    if (!targetType || cost <= 0) {
      return NextResponse.json({ success: false, error: { code: 'INVALID_INPUT', message: 'targetType and cost are required' } }, { status: 400 });
    }
    const db = getAdminDb();
    await db.collection(COLLECTION).doc(targetType).set({ cost, updatedAt: new Date().toISOString(), updatedBy: decoded.uid }, { merge: true });
    return NextResponse.json({ success: true, data: { cost, targetType } });
  } catch (error) {
