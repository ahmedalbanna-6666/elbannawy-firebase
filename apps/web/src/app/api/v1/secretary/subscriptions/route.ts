    ...
    return NextResponse.json(
      {
        ...
        error: {
          code: "INVALID_INPUT",
          message: "Provide studentId, expiringWithin, or status=expired",
        },
      },
      { status: 400 },
    );
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (body.action === "create" || !body.action) {
      if (!body.studentId || !body.planId) {
        return NextResponse.json(
          {
            success: false,
            error: { code: "INVALID_INPUT", message: "studentId and planId are required" },
          },
          { status: 400 },
        );
      }
    ...
    return NextResponse.json(
      { success: false, error: { code: "INVALID_INPUT", message: "Unknown action" } },
      { status: 400 },
    );
  } catch (error) {
    return NextResponse.json(
