...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const result = await liveRepo.listDateBlocks(teacherId);
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (role !== 'teacher' && role !== 'admin') {
      return NextResponse.json({ success: false, error: { code: 'FORBIDDEN', message: 'Only teachers and admins can manage date blocks' } }, { status: 403 });
    }
