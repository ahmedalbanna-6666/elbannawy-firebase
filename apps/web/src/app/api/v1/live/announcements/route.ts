...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const result = await liveRepo.listAnnouncements(sessionId);
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (role !== 'teacher' && role !== 'admin') {
      return NextResponse.json({ success: false, error: { code: 'FORBIDDEN', message: 'Only teachers and admins can create announcements' } }, { status: 403 });
    }
    ...
    const result = await liveRepo.createAnnouncement(body as any);
