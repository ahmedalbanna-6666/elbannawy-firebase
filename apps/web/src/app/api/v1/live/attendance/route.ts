...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const result = await liveRepo.listAttendance(sessionId);
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (role !== 'teacher' && role !== 'admin') {
      return NextResponse.json({ success: false, error: { code: 'FORBIDDEN', message: 'Only teachers and admins can record attendance' } }, { status: 403 });
    }
    ...
    const result = await liveRepo.recordAttendance(body as any);
