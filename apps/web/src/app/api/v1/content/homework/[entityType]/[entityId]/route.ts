...
const COLLECTION = 'contentHomework';
...
export async function GET(
  ...
  try {
    ...
    const homework = { id: snap.docs[0].id, ...snap.docs[0].data() };
...
export async function POST(
  ...
  try {
    ...
    const id = `chw_${Date.now()}`;
    const now = new Date().toISOString();
    const doc = {
      id, entityType: entityType.toUpperCase(), entityId,
      title: body.title ?? 'Untitled Homework',
...
export async function DELETE(
  ...
  try {
    ...
    const snap = await db.collection(COLLECTION)
      ...
      .limit(1)
      .get();
    if (snap.empty) return NextResponse.json({ success: false, error: { code: 'NOT_FOUND', message: 'Homework not found' } }, { status: 404 });
