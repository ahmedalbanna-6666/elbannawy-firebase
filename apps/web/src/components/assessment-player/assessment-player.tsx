...
export interface AssessmentTypeConfig {
  ...
  errorTitle: string;
  showXP?: boolean;
  showPrereqCheck?: boolean;
  showNextLessonBadge?: boolean;
}
...
export function AssessmentPlayer({ lessonId, type: t }: AssessmentPlayerProps): ReactNode {
  ...
  const fetchAssessment = useCallback(async (): Promise<void> => {
    try {
      setError(null);
      setPrereqError(null);
      const [aRes, qRes, resultRes] = await Promise.all([
        api.get<AssessmentData>(`/${t.apiPrefix}/${lessonId}`),
        api.get<{ questions: AssessmentQuestion[] }>(`/${t.apiPrefix}/${lessonId}/questions`),
        api.get<AssessmentResult>(`/${t.apiPrefix}/${lessonId}/result`),
      ]);
      ...
      if (err instanceof Error) {
        if (t.showPrereqCheck && err.message.includes("403")) {
  ...
  }, [lessonId, t.apiPrefix, t.showPrereqCheck]);
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = setInterval(() => {
      ...
      if (saveData.length > 0) {
        void api.patch(`/${t.apiPrefix}/${lessonId}/save`, { answers: saveData });
  ...
  }, [answers, questions, lessonId, result, viewingReview, t.apiPrefix]);
  ...
  const handleStartAttempt = async (): Promise<void> => {
    try {
      setPrereqError(null);
      await api.post(`/${t.apiPrefix}/${lessonId}/start`);
      setResult(null);
      setReview(null);
      ...
      if (err instanceof Error && t.showPrereqCheck && err.message.includes("403")) {
        setPrereqError(err.message);
      } else {
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    try {
      ...
      const res = await api.post<AssessmentResult>(`/${t.apiPrefix}/${lessonId}/submit`, {
      ...
      setError(err instanceof Error ? err.message : `فشل تسليم ${t.titleSingular}`);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleViewReview = async (): Promise<void> => {
    try {
      const res = await api.get<ReviewData>(`/${t.apiPrefix}/${lessonId}/review`);
      if (res.data) {
        setReview(res.data);
  ...
  if (error) return <ErrorState title={t.errorTitle} description={error} />;
  if (!assessment) {
    return (
      <EmptyState
        title={t.noDataTitle}
        description={t.noDataDescription}
        icon={<t.icon className="h-16 w-16" />}
      />
    );
  }

  if (t.showPrereqCheck && prereqError) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20">
        <Lock className="h-16 w-16 text-warning-500" />
        <h2 className="text-xl font-bold text-neutral-900 dark:text-neutral-100">{t.lockedTitle}</h2>
      ...
      <div>
        ...
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-neutral-900 dark:text-neutral-100">{assessment.title}</h1>
            <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-neutral-500">
              <span className="flex items-center gap-1">
                <t.icon className="h-4 w-4" />
                {assessment._count.questions} سؤال
              </span>
              <span className="flex items-center gap-1">
                <Trophy className="h-4 w-4" />
                نسبة النجاح {assessment.passingScore}%
              </span>
              <span>الحد الأقصى {assessment.maxAttempts} محاولات</span>
              {t.showXP && assessment.xpReward > 0 && (
                <span className="flex items-center gap-1">
          ...
          <CardContent>
            <div className="flex flex-col items-center gap-3 text-center">
              ...
              <div>
                ...
                {t.showNextLessonBadge && result.nextLessonUnlocked && (
...
function AssessmentSkeleton(): ReactNode {
  return (
    <div className="flex flex-col gap-6">
