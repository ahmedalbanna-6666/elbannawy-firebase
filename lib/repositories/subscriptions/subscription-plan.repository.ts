...
import type { ISubscriptionPlanRepository, ISubscriptionPlan, ISubscriptionPlanFilter } from '../contracts';
...
export class SubscriptionPlanRepository implements ISubscriptionPlanRepository {
  ...
  async getById(id: string): Promise<RepositoryResult<ISubscriptionPlan | null>> {
    ...
    } catch (error) { return { ok: false, error: { ...toRepositoryError(error) } } as unknown as RepositoryResult<ISubscriptionPlan | null>; }
  }

  async list(filter?: ISubscriptionPlanFilter): Promise<RepositoryResult<ISubscriptionPlan[]>> {
    try {
      let query = this.db().collection(COLLECTION).orderBy('sortOrder', 'asc') as FirebaseFirestore.Query;
    ...
    } catch (error) { return { ok: false, error: { ...toRepositoryError(error) } } as unknown as RepositoryResult<ISubscriptionPlan[]>; }
  }

  async update(id: string, input: Partial<ISubscriptionPlan>): Promise<RepositoryResult<ISubscriptionPlan>> {
    try {
