...
function extractActivity(section: {
  ...
  switch (section.type) {
    case 'MCQ':
      content = extractMcq(section.rawText);
      break;
    case 'DRAG_DROP':
      content = extractDragDrop(section.rawText);
      break;
    case 'READING':
      content = extractReading(section.rawText);
      break;
    case 'REWRITE':
      content = extractRewrite(section.rawText);
      break;
    case 'CORRECT':
      content = extractCorrect(section.rawText);
      break;
    case 'DIALOGUE':
      content = extractDialogue(section.rawText);
      break;
    case 'TRUE_FALSE':
      content = extractTrueFalse(section.rawText);
      break;
    case 'WRITING':
      content = extractWriting(section.rawText);
      break;
    default: {
  ...
  for (const activity of activities) {
    ...
    if (activity.type === 'MCQ') {
      ...
      for (const cat of mcq.categories) {
        for (const q of cat.questions) {
          if (q.options.length < 2) {
            result.push(error('MCQ_TOO_FEW_OPTIONS', `MCQ #${activity.order} question ${q.number}: has ${q.options.length} options, minimum is 2`));
          }
        }
      }
      ...
      for (const n of qNumbers) {
        if (seen.has(n)) {
          result.push(error('DUPLICATE_QUESTION_NUMBER', `MCQ #${activity.order}: duplicate question number ${n}`));
        }
        seen.add(n);
      }
    }
      ...
      if (questions) {
        ...
        for (const q of questions) {
          if (seen.has(q.number)) {
            result.push(error('DUPLICATE_QUESTION_NUMBER', `${activity.type} #${activity.order}: duplicate question number ${q.number}`));
          }
          seen.add(q.number);
        }
      }
...
export async function importQuestionsFromDocx(options: ImportOptions): Promise<ImportResult> {
  ...
  let stat: fs.Stats;
  try {
    stat = fs.statSync(options.filePath);
  } catch {
  ...
  try {
    const result = await mammoth.extractRawText({ path: options.filePath });
...
export function parseQuestionText(rawText: string): ImportResult {
  ...
  for (const section of sections) {
    ...
    for (const err of activity.errors) {
      errors.push(err);
    }
  }
  ...
  const documentTitle = header.split('\n')[0]?.trim() || 'Untitled Document';
