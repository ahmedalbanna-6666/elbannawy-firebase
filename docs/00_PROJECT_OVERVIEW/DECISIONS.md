...
Architecture:
- Firebase Authentication replaces JWT + Passport for identity
- Cloud Firestore replaces PostgreSQL + Prisma for operational storage
- Cloud Storage for Firebase replaces Cloudflare R2 / MinIO
- pgvector remains as the independent AI vector database service
- NestJS backend remains during migration; new features target Firestore
- lib/repositories/ contains the Firestore repository implementations

The migration follows the phased plan in ARCHITECTURE_LOCK.md.

Status: Approved (Phase 1 Documentation Complete, Phase 2 In Progress)
